    ...
    bills.forEach(bill => {
      ...
      if (shouldGenerate) {
        const nextDueDate = dueDate.add(
          bill.recurring === 'monthly' ? 1 : 
          bill.recurring === 'quarterly' ? 3 : 
          12, 'month'
        )
            ...
            <form onSubmit={handleSubmit}>
              <Stack spacing={3}>
                ...
                <DatePicker
                  ...
                  onChange={(date) => setNewBill({...newBill, dueDate: date?.format('YYYY-MM-DD') || ''})}
                ...
                {isRecurring && (
                  <TextField
                    ...
                    fullWidth
                  >
                    {recurringOptions.filter(opt => opt.value !== '').map(option => (
